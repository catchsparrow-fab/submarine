...
inline half3 ComputeDepthColor(half3 posWorld, half3 eyeVec, half3 lightDir, half3 lightColor, float maxDepth = 0)
{
...
#endif
	{
		...
		if (depth < maxDepth)
			light1 = (1.0 + tex2D(_CausticsMap, mul(_CausticsMapProj, samplePoint) * half2(0.5, -0.5) + 0.5).rrr * 0.5) * saturate((maxDepth - depth) / step);						// caustic rays
		else
			light1 = 0;
#endif
...
inline half3 ComputeRefractionColor(half3 normalWorld, half3 eyeVec, half3 posWorld, UnityLight light, WaterData waterData, out half3 depthFade)
{
...
#if SHADER_TARGET >= 30
	half centerSceneDepth = LinearEyeDepthHalf(SAMPLE_DEPTH_TEXTURE_PROJ(_WaterlessDepthTexture, waterData.grabPassPos).r) - waterSurfaceDepth;
	...
	half distortScale = 1.0;
		...
		half4 refractCoord = UNITY_PROJ_COORD(waterData.grabPassPos + ComputeDistortOffset(normalWorld, _RefractionDistortion) * distortScale);
	#else
		half4 refractCoord = UNITY_PROJ_COORD(waterData.grabPassPos);
	#endif

#if !defined(_WATER_BACK)
	half sceneDepth = LinearEyeDepthHalf(SAMPLE_DEPTH_TEXTURE_PROJ(_WaterlessDepthTexture, refractCoord).r) - waterSurfaceDepth;
