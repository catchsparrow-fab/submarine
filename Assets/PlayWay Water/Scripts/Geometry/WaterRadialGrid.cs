	...
	public class WaterRadialGrid : WaterPrimitiveBase
	{
		...
		protected override Matrix4x4 GetMatrix(Camera camera)
		{
			...
			if(camera.orthographic)
				scale = new Vector3(camera.farClipPlane, camera.farClipPlane, camera.farClipPlane);
			else
				scale = new Vector3(camera.farClipPlane * Mathf.Tan(camera.fieldOfView * 0.5f * Mathf.Deg2Rad) * camera.aspect + water.MaxHorizontalDisplacement * 2, camera.farClipPlane, camera.farClipPlane);
